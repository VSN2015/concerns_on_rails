...
A relation's total is a `COUNT` with `ORDER`/`LIMIT`/`OFFSET` stripped; a `DISTINCT` relation with its own `SELECT` list (`Membership.select(:group_id).distinct`) is counted as a raw `SELECT COUNT(*) FROM (…)` subquery (STI-safe; MySQL falls back to reading the distinct rows when the select list repeats an output name), so the total is the number of distinct rows it returns, not the underlying row count.
...
**Notes**
- The primary key is always appended as a tiebreaker, so duplicate values never skip or repeat rows; ordering columns are chosen **in code** (never from params) and should be `NOT NULL` — if the relation contains any row with a NULL in an ordering column, every page raises `ArgumentError` rather than silently dropping rows (one indexable `EXISTS … IS NULL` probe per schema-nullable ordering column per page; NOT NULL columns cost nothing; exclude NULLs with `where.not(col: nil)`). Boundary values are the stored column values (`read_attribute`), so an overridden attribute reader cannot derail the walk. Re-declaring `cursor_paginate_by` (e.g. in a subclass) changes only the options passed — the rest, `signed:` included, are inherited.
