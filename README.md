...
- Re-declaring a field (later on the same class, or on an STI subclass) **merges** onto its current config, so only the options you pass change: `sequenceable_by :sequence, assign: :manual` on a subclass keeps the inherited `into:` / `prefix:` / `reset:`. A re-declaration that changes only `assign:` and/or `time_zone:` (or repeats the inherited values) keeps drawing from the **inherited counter**, so it never reissues a parent number in the same format. Changing a format option (`prefix`, `template`, `padding`, `reset`, `scope`, `into`, `separator`, `start_at`) starts the subclass's own series, as described below.
