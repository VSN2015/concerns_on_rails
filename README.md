...
A relation's total is a `COUNT` with `ORDER`/`LIMIT`/`OFFSET` stripped; a `DISTINCT` relation with its own `SELECT` list (`Membership.select(:group_id).distinct`) is counted as a subquery, so the total is the number of distinct rows it returns, not the underlying row count.
...
**Notes**
- The primary key is always appended as a tiebreaker, so duplicate values never skip or repeat rows; ordering columns are chosen **in code** (never from params) and should be `NOT NULL` — a NULL on the page-boundary row, on the row just after it, or on a row the last page of a walk would skip raises `ArgumentError` rather than silently dropping rows. Boundary values are the stored column values (`read_attribute`), so an overridden attribute reader cannot derail the walk. Re-declaring `cursor_paginate_by` (e.g. in a subclass) changes only the options passed — the rest, `signed:` included, are inherited.
...
**Options**: `*actions` (none = catch-all). Lookup is by **specificity**: a rule naming the action always beats a catch-all, whichever class declared either; ties go to the most-derived declaring class, then declaration order. So a parent's catch-all never shadows a subclass rule, and a subclass catch-all for its new actions never takes over (or strips the `replay:`/`tolerance:` of) an action its parent named. `secret:` (String, callable resolved per request — a Proc is `instance_exec`'d, any other callable is passed the controller — or Array for rotation, any match passes), `scheme:` (`:hex`), `header:` (overrides the preset), `tolerance:` (Stripe only, `300`s default), `digest:` (`:sha256`; `:sha1`/`:sha512` for `:hex`/`:base64` only), `replay:` (`true` = the gem-wide `cache_store`, a store object, or `false`/`nil` for off) + `replay_ttl:` (`24.hours`) — replay protection for the schemes that carry no timestamp.
