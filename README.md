...
| `time_zone:`         | app zone    | Zone the `reset:` periods are cut in (a zone name or `ActiveSupport::TimeZone`). Defaults to `config.time_zone` (`Time.zone_default`), else UTC — never the per-request `Time.zone`. Unknown names raise `ArgumentError`. |
...
| `assign:`            | `:create`   | `:create` numbers every record in `before_create`; `:manual` leaves the column NULL until `assign_<field>!` is called — for invoices that get their number when finalized, not when drafted. Re-declaring the field (same class or an STI subclass) switches the mode; the last declaration wins. |
...
- Re-declaring a field (later on the same class, or on an STI subclass) **merges** onto its current config, so only the options you pass change: `sequenceable_by :sequence, assign: :manual` on a subclass keeps the inherited `into:` / `prefix:` / `reset:`. **One visible format = one counter and one zone.** After merging, the subclass's full format (`prefix`, `template`, `padding`, `reset`, `scope`, `into`, `separator`, `start_at`) is compared with each inherited owner's config, walking up the chain. If one is identical, the subclass keeps drawing from that **inherited counter**. That holds even after a subclass went `CN-` and back to `INV-`, so it never reissues a parent number. Only a format no ancestor uses starts the subclass's own series, as described below. `assign:` is not part of the format. Keeping the format but changing `time_zone:` (under `reset:`) raises `ArgumentError`: the two classes would cut the same day at different instants and issue the same number. Siblings that declare the same format without a declaring ancestor can't be detected, so give them distinct prefixes or `scope: :type`.
