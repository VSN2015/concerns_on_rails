...
| `by:` | callable (`Proc`, `lambda`) | `-> { request.remote_ip }` | Discriminator. A Proc/lambda is evaluated with `instance_exec` on the controller instance, so `request`, `params`, and helpers such as `current_user` are all in scope; any other callable (an object with `#call`, a `Method`) is passed the controller — or called bare when its `#call` takes no arguments. Must respond to `#call`; a plain string or symbol raises `ArgumentError`. |
