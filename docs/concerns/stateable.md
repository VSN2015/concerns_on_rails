...
- **`ArgumentError` at class load time** — `stateable_by` validates eagerly. Errors are raised when the class is loaded, not at runtime. The following all raise `ArgumentError`:
  ...
  - A generated method (`<state>?`, `<state>!`, `<event>!`, `may_<event>?`) or scope (`.<state>`) would override one the class already has from ActiveRecord or another concern — an event named `lock` (AR's pessimistic `lock!`, which `with_lock` and `lock: true` rely on), a state named `valid`, an event `restore` next to `SoftDeletable#restore!`, a state `active` next to `Activatable`. Use `prefix:` or `suffix:`. Methods an earlier `stateable_by` generated are exempt, so re-declaring on the same class or a subclass works.
...
- **Method name conflicts are refused** — a state or event whose generated method or scope would override an existing ActiveRecord method (`valid?`, `lock!`, `save!`, …) or one another concern defined (`restore!`, `.active`) raises `ArgumentError` at class load instead of silently shadowing it. Use `prefix:` or `suffix:`. (Column attribute methods Rails generates are not checked, wherever they live.) The reverse order is refused too: Activatable, Expirable, Lockable, Anonymizable, Publishable, SoftDeletable and Schedulable raise when included (or when their macro defines a scope) after a `stateable_by` whose generated methods or scopes they would replace or be shadowed by, and Storable's own collision guard catches Stateable's methods. Other concerns included after `stateable_by` are not checked in that order.
