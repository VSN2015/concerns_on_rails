...
- **No callbacks, no instance state.** The concern adds four instance methods (`render_success`, `render_created`, `render_invalid`, `render_error`) and one optional class macro (`respondable_by`); there are no `before_action` hooks or instance variables.
