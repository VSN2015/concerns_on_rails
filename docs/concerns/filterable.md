...
- **Casting follows ActiveRecord.** `gt`/`gte`/`lt`/`lte` values are cast through the column's attribute type (so `"10"` compares as a decimal against a decimal column, and SQLite's type affinity cannot bite), or through `type:` when given. Equality, `not`, `in` and `null` go through `where`, which casts on its own.
- **An uncastable comparison value matches nothing.** `?price_gte=abc` on a numeric column, or `?created_at_lt=yesterday` on a datetime one, returns **no rows** rather than a silently wrong set — `ActiveModel::Type::Integer#cast("twelve")` is `0`, not `nil`, so casting the garbage anyway would have turned `?stock_gt=twelve` into `stock > 0` and handed back rows the caller never asked for. This is the same answer `where` already gives: `Integer#serialize` (unlike `#cast`) returns `nil` for a non-numeric string, so plain equality, `not` and `in`/`not_in` emit `= NULL` / `!= NULL` and match nothing too — the comparison operators just have to reach it by hand, because pre-casting for Arel bypasses that guard. Nothing raises at request time; if you want a 400 on a malformed filter, validate the params with Permittable first.
- **`contains`/`starts_with` escape LIKE wildcards** (`%`, `_`, `\`) and emit an explicit `ESCAPE '\'` clause, exactly like Searchable. **Matching is case-insensitive on all three supported adapters**, not just PostgreSQL: Arel emits `ILIKE` on PostgreSQL, while SQLite's `LIKE` folds ASCII case by default and MySQL's folds case under the usual `_ci` collations. There is no `case_sensitive:` option here — reach for `Searchable` (`case_sensitive: true`) when the distinction matters.
