...
| `unlock_token:` | `Symbol` or `nil` | `nil` | A string column. When set, `lock_access!` (including the lock tripped by `register_failed_attempt!`) mints a 43-character URL-safe token into it in the same write, and every unlock path — `unlock_access!`, `unlock_by_token`, `unlock_expired`, the quiet stale-lock reset on a failed attempt — clears it. The token is honoured only while the lock is live, so `unlock_in:` doubles as the link's TTL. Must differ from `attempts:`/`locked_at:`; the column is checked at class load and registered with Rails' `filter_parameters`. |
...
| `lock_access! → true` | Locks now with one **conditional `UPDATE`** (validations/callbacks bypassed) that only applies while the row is unlocked in the database, so two requests crossing the threshold at once lock it exactly once: the loser adopts the winner's lock and unlock token (never re-minting the mailed link) and fires no hooks. Idempotent while locked; an expired lock is re-locked with a fresh timestamp. Runs `before_lock`/`after_lock` in a transaction, after the row is claimed — a hook that raises or vetoes with `ActiveRecord::Rollback` undoes the lock. Like `update_columns`, raises `ActiveRecord::ReadOnlyRecord` on a `readonly!` record and `ActiveRecord::ActiveRecordError` on a destroyed one; returns `false` when the row has been deleted. |
...
| `User.unlock_by_token(token) → record \| nil` | _(class method, needs `unlock_token:`)_ Finds the row holding `token` (through the model's default scope, then a constant-time compare on the fetched value), claims the token with a conditional `UPDATE` so concurrent clicks on the same link cannot both win, then runs `unlock_access!` — hooks fire, counter zeroed, token cleared — and returns the record. `nil` for a blank, unknown or already-used token, and for a token whose lock has already lapsed under `unlock_in:` (that token is retired on the spot). A hook vetoing the unlock with `ActiveRecord::Rollback` puts the token back, so the link stays usable. Raises `ArgumentError` when `unlock_token:` is not configured. |
...
- **Unlock tokens are single-use and live exactly as long as the lock.** The token is minted with the lock and cleared by every unlock path, so a mailed link works once; `after_lock` is where you send it. With `unlock_in:` set, a lock that lapses by itself takes its token with it — `unlock_by_token` refuses the late click (the account is already usable) and retires the row's dangling token, so `unlock_in:` is the link's TTL. With `unlock_in: nil` the lock is permanent and so is the link, exactly as in Devise. Use a unique index on the column (the `string:uniq` hint) — `unlock_by_token` does an indexed equality lookup before its constant-time compare. The column is registered with Rails' `filter_parameters`, but it is **not** hidden from `as_json`: exclude it yourself if you serialize the model.
