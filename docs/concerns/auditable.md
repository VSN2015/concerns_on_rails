...
### `auditable_by(*fields, into: :audit_log, actor: nil, max_entries: 200)`
...
- **Bounded by design.** The default `max_entries: 200` keeps the row from growing without limit; the oldest entries are silently trimmed. Pass `nil` only when you have an external cleanup story.
