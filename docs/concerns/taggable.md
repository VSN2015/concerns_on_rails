...
Matching is also **case-insensitive on every adapter** — `LIKE` on SQLite and MySQL, `ILIKE` on PostgreSQL — so `tagged_with("elixir")` finds a record tagged `"Elixir"` wherever you run it. The Ruby-side helpers (`tagged_with?`, `all_tags`, `tag_counts`) compare exactly; `downcase: true` folds on write and is what makes the scope and the helpers agree.
