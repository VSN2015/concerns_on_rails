...
| `prefix:` / `suffix:` | `Symbol` / `true` | `nil` | Affix the scope names (`.term_active`, …) **and** define affixed predicates (`term_active?`, `term_expired?`) that always answer Expirable's question. `true` uses the field name. |
...
| `expire!(time = Time.zone.now)` | Persists an expiry timestamp via `update`. Defaults to the current time, making the record immediately expired; `nil` or a blank value (`""`, `false`, `[]`) also means now. Accepts a `Time`/`Date` or a parseable String; anything the column type cannot cast to a time raises `ArgumentError` before any hook runs. Runs `before_expire`, the write, then `after_expire` (only when the write succeeded) in their own savepoint. Returns `true` only once `after_expire` has returned: a failed write or a hook's `ActiveRecord::Rollback` returns `false` and writes nothing, even inside a caller's transaction. A time in the future only schedules expiry, so the hooks do not fire. |
| `expire_in!(duration)` | Sets an absolute lifetime from now — `expire!(Time.zone.now + duration)` — whatever the current expiry. A positive duration schedules expiry, so no hooks fire. Raises `ArgumentError` for a non-duration. |
| `clear_expiry!` | Sets the expiry column to `nil` so the record never expires. No hooks (nothing expired). |
| `before_expire` / `after_expire` | Lifecycle hooks, no-ops by default. Fired when a write actually expires the record (`expire!` with a past-or-now time, `expire_all`); not when one merely schedules expiry (a future `expire!`, `expire_in!`), nor by `extend_expiry!` or `clear_expiry!`. Overriding either moves `expire_all` to the per-record path. |
...
- **Hooks share the write's savepoint.** A raising `after_expire`, or one calling `raise ActiveRecord::Rollback`, rolls the expiry back. This holds even inside a caller's transaction, and `expire_all` then raises `ActiveRecord::RecordNotSaved` and rolls back the batch. A write that fails validation returns `false`, skips `after_expire`, and rolls back `before_expire`'s side effects. `expire_all` detects an overridden hook (like an overridden `expire!`) and streams per record so the hooks run for every row — declare hooks knowing the bulk verb pays a query per record.
...
- **`nil` means never-expires, not immediately-expired.** `expired?` returns `false` and `active?` returns `true` when the column is `nil`. The `.active` scope includes `NULL` rows; the `.expired` scope excludes them. (A `nil` *argument* to `expire!` / `expire_all` is different: it means now. Use `clear_expiry!` to make a record never expire.)
- **Predicate collisions.** Activatable also defines `active?`, and Schedulable defines `expired?`. On a model that combines them, the concern included last owns the plain name. With `prefix:`/`suffix:` configured, use the affixed predicates (`term_active?` / `term_expired?`), which always give Expirable's answer.
