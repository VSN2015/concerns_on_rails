...
- **A boolean column's `.published` is `<> FALSE`, not `= TRUE`.** Rails copies an equality condition from a scope onto records built through it. So a `default_scope: true` built on `where(is_published: true)` made every new record start out published. `<> FALSE` selects the same rows (`NULL` stays out) and is never copied, so new records start unpublished. That includes `Post.published.new`.
