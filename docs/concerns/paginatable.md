...
- **`DISTINCT` over a custom `SELECT` is counted as a subquery.** `select(:group_id).distinct` returns one row per distinct value, so stripping its `SELECT` list (as the plain count does) counted every underlying row — 6 rows over 3 groups reported `X-Total-Count: 6` and advertised empty pages. Such a relation is counted as `SELECT COUNT(*) FROM (SELECT DISTINCT ...)`, portable across adapters for any number of columns (unlike `COUNT(DISTINCT a, b)`). A plain `.distinct` (no `SELECT` list) and grouped relations are counted as before.
