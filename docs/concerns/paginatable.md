...
- **`DISTINCT` over a custom `SELECT` is counted as a subquery.** `select(:group_id).distinct` returns one row per distinct value, so stripping its `SELECT` list (as the plain count does) counted every underlying row — 6 rows over 3 groups reported `X-Total-Count: 6` and advertised empty pages. Such a relation is counted as raw SQL, `SELECT COUNT(*) FROM (SELECT DISTINCT ...)` — portable across adapters for any number of columns (unlike `COUNT(DISTINCT a, b)`), and not re-scoped by the model (an STI subclass's `type` condition stays inside the subquery). MySQL rejects a derived table whose select list repeats an output name (`select("a.id, b.id")`), so the list is re-aliased positionally inside it (`... AS c0, ... AS c1` — the same expressions, so the same distinct rows). A list that cannot be split safely (`table.*`) keeps its own names; should MySQL still reject it, the total falls back to the plain (over-)count — still one `COUNT`, never the rows loaded into memory. A plain `.distinct` (no `SELECT` list) and grouped relations are counted as before.
- **Non-finite JSON numbers are junk.** A JSON body's `{"page": 1e400}` decodes to `Float::INFINITY` (and `.to_i` on it raised `FloatDomainError` — a 500); Infinity and NaN now fall back to the default `page`/`per_page` like any other garbage.
