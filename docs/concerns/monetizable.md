...
| `unit:` | `String` | `"$"` | Currency symbol prepended to the formatted string. Must not contain the `delimiter:` or `separator:` (raises `ArgumentError` at class load), since the writer could not tell them apart when reading formatted output back. |
...
| `price=(amount)` | — | Multiplies `amount` by `subunit_to_unit`, rounds to the nearest whole subunit, and writes the result back to the integer column. Accepts a number or a String. A String first has the configured `unit:` removed — only at the start (after an optional sign) or the end, so `"5$5"` is garbage — and is then read as a plain decimal (`"19.99"`, as before) or in the field's own display format — `delimiter:` (only where it groups digits in threes) and `separator:` — so `formatted_price` output reads back (`"$1,234.50"`, `"-$5.00"`, `"€1.234,50"` for `separator: ","`). In a field that groups with `"."` and separates with something else, `"1.234"` / `"1.234.567"` are grouped thousands (the same amount as `"€1.234"`); other `"."` decimals (`"19.99"`, `"1.5"`) are still read canonically. Surrounding Unicode whitespace is ignored. Assigns `nil` when `amount` is `nil`, garbage (`"abc"`, a wrong-locale `"1,5"` in a `"."` field), non-finite (`Float::NAN`, `Float::INFINITY`), or oversized — longer than 64 characters, a written exponent of three or more digits (`"1e100000000"`), or a magnitude of 10^24 major units or more — so the setter never raises and cannot be made to burn CPU. The 10^24 cap only bounds parsing work; it is far above what an integer-subunit column holds (a 64-bit `bigint` tops out at ~9.2e18 subunits, about 9.2e16 major units at `subunit_to_unit: 100`; a 32-bit `integer` at ~2.1e9, about 21 million), so the database or ActiveRecord (`ActiveModel::RangeError`) may still reject a large amount at save — unchanged from before. The exponent check runs after the unit is removed and needs a digit before the `e`, so a unit ending in e/E (`"Le100.00"`, `"CHE100.00"`) reads back normally. |
