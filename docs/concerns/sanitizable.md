...
- **`:strip` on write stores plain text.** Entities the sanitizer introduces are decoded (`"<b>Tom</b> & Jerry"` → `"Tom & Jerry"`, not `"Tom &amp; Jerry"`, which the view would double-escape), EXCEPT `&lt;` and `&gt;`: they stay encoded so the stored value can never become markup, even when rendered with `raw` — a typed `a < b` is stored as `a &lt; b`. An `&amp;` whose bare `&` would read back as a character reference (the literal text `&amp;copy;`) also stays encoded, so re-saving and `sanitize_all!` are idempotent. That decision is a static, linear-time rule, not a re-parse: an `&` followed by `#`, by a well-formed `name;`, or by one of HTML5's semicolon-less legacy names (`copy`, `lt`, `not`, …) keeps its `&amp;` — so ampersand-heavy input cannot make a save slow. A carriage return (reachable only as `&#13;`) is stored as `\n`. `sanitize_all!` stores the same plain-text form (an explicitly named `on: :read` `:strip` column included) and repairs rows stored double-escaped by earlier versions. The `sanitized_<field>` reader, `sanitized_attributes` and `as_json(sanitized: true)` still return the HTML-escaped form.
