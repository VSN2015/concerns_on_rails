...
| `sanitize_all!(*fields)` | Rewrites stored values in place for every record in the current scope (`Article.where(...).sanitize_all!`), issuing one `update_columns` per row whose values actually change, inside a transaction. A bare call runs only the `on: :write` fields' sanitizers — the rows this tool exists to repair; name fields explicitly to include an `on: :read` column. Returns the Integer count of rows rewritten; idempotent, and `0` without any query when nothing is declared `on: :write`. Deliberately skips validations and callbacks (like Anonymizable) — it is a repair tool for rows written via `update_column`/`update_all`/raw SQL or before the concern existed. A row that disappears mid-sweep raises `ActiveRecord::RecordNotSaved` and rolls the whole batch back (the shared `Support::BatchOps` contract). An undeclared field raises `ArgumentError`. |
...
- **`:strip` on write stores plain text.** Entities the sanitizer introduces are decoded (`"<b>Tom</b> & Jerry"` → `"Tom & Jerry"`, not `"Tom &amp; Jerry"`, which the view would double-escape), EXCEPT `&lt;` and `&gt;`: they stay encoded so the stored value can never become markup, even when rendered with `raw` — a typed `a < b` is stored as `a &lt; b`. An `&amp;` whose bare `&` would read back as a character reference (the literal text `&amp;copy;`) also stays encoded, so re-saving and `sanitize_all!` are idempotent. That decision is a static, linear-time rule, not a re-parse: an `&` followed by `#`, by a well-formed `name;`, or by one of HTML5's semicolon-less legacy names (`copy`, `lt`, `not`, …) keeps its `&amp;` — so ampersand-heavy input cannot make a save slow. Carriage returns are stored as `\n`: under the HTML5 parser (the default) that only changes a `&#13;`, since the parser already turns a raw CRLF into LF; on the HTML4 fallback (JRuby, no libgumbo) a raw CRLF in the value is now stored as LF too. `sanitize_all!` stores the same plain-text form (an explicitly named `on: :read` `:strip` column included) and repairs rows stored double-escaped by earlier versions. The `sanitized_<field>` reader, `sanitized_attributes` and `as_json(sanitized: true)` still return the HTML-escaped form.
...
- **`sanitized:` rides `serializable_hash`.** The override calls `super` first, so `only:`/`except:`/`methods:`/`include:` behave exactly as in Rails; only the values of declared fields still present in the hash are replaced, and they are sanitized from the SERIALIZED value, so an overridden reader or a `Maskable` field is sanitized after its own filtering — `as_json(masked: true, sanitized: true)` keeps the mask in either include order. Nested `include:` records inherit the request: `sanitized: true` is merged into every include entry that does not set it explicitly, so a Sanitizable child is cleaned too. Pass `include: { comments: { sanitized: false } }` to opt a child out. Children sanitize all of their own declared fields even when the parent named a subset, because a field list names the parent's columns.
- **`sanitize_all!` keeps an Encryptable blind index in step.** `update_columns` skips `before_save`, so the concern writes the refreshed `<field>_bidx` fingerprint itself in the same UPDATE. Without it `find_by_<field>` would keep resolving the record by the markup that is no longer stored.
