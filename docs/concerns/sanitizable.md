...
| `sanitize_all!(*fields)` | Rewrites stored values in place for every record in the current scope (`Article.where(...).sanitize_all!`), running each declared field's sanitizer (all declared fields by default, or the given subset) and issuing one `update_columns` per row whose values actually change, inside a transaction. Returns the Integer count of rows rewritten; idempotent. Deliberately skips validations and callbacks (like Anonymizable) — it is a repair tool for rows written via `update_column`/`update_all`/raw SQL or before the concern existed. An undeclared field raises `ArgumentError`. |
...
- **`sanitize_all!` writes with `update_columns`.** No validations, no callbacks, no `updated_at` bump — deliberately, so a repair can't be blocked by an unrelated validation or trigger side effects (an Auditable capture, a webhook). It rewrites `:read`-mode fields too, so only call it for fields you actually want stored clean.
