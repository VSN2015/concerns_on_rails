...
- **Store outages fail closed.** Rails' Redis and memcached stores swallow connection errors — `#write` returns false and `#read` returns nil — which looks exactly like "someone else holds the key". A lost claim with nothing to read back is therefore retried once: a lock that merely expired in between is won and the action runs; a second failure means the store is down, and the request gets 503 `idempotency_store_unavailable` (not the permanent 409 it used to get). The default is `:reject` because this concern exists for requests where a duplicate is harmful — with the store down there is no way to know whether a retry's original already ran, and a 503 the client retries later costs only latency, where running it risks a double charge. Use `on_store_unavailable: :proceed` on endpoints where a duplicate is merely noise.
...
- **An unreachable store answers 503, not 409.** A falsy claim write followed by a nil read used to be treated as "in flight", so during a cache outage every keyed request got `409 idempotency_conflict` indefinitely. The claim is now retried once, and a second failure renders `503 idempotency_store_unavailable` with `Retry-After` (Respondable-aware). New option `on_store_unavailable:` (`:reject` default, `:proceed`).
