...
- **Include order vs Throttleable.** Include `Throttleable` first so a 429 halts the request before an idempotency claim is written.
...
- **Keys are validated, then hashed.** A key must be non-blank and at most 255 characters (else 400); the cache key uses its SHA256, while the raw key is echoed in `X-Idempotency-Key`.
