...
- **Nested attributes.** `accepts_nested_attributes_for :alias` behaves exactly like it does for the source — create, update, `_destroy` (with `allow_destroy:`), `reject_if:` and `limit:` all work, and each child is saved once. It turns autosave on for the **source** association (both names share one cache, so they cannot differ); only `<alias>_attributes=` is generated, and child validation errors are keyed under the source name (e.g. `books.title`). Declared in a **subclass**, it sets autosave on the *inherited* source reflection — the object is shared with the parent, so the parent class autosaves that association too. That is exactly what stock Rails `accepts_nested_attributes_for` does for an inherited association; declare it on the parent to make that explicit.
