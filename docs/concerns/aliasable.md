...
- **Nested attributes.** `accepts_nested_attributes_for :alias` behaves exactly like it does for the source — create, update, `_destroy` (with `allow_destroy:`), `reject_if:` and `limit:` all work, and each child is saved once. It turns autosave on for the **source** association (both names share one cache, so they cannot differ); only `<alias>_attributes=` is generated, and child validation errors are keyed under the source name (e.g. `books.title`).
