...
| `required:` | Array of Symbols | `%i[line1 city postal_code country]` | Canonical part names that must be present on save. Each listed part must resolve to a column that exists in the schema; otherwise an `ArgumentError` is raised at load time. Declared while the schema is unreachable (`db:create`, an unmigrated table, `assets:precompile`), the model still loads: the column mapping is resolved — and override columns and required parts checked — on first use once the table exists, mapping nothing until then. Pass `[]` to make all parts optional. |
