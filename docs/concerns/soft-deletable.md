...
| `restore!` | Clears the soft-delete column (sets it to `nil`). Returns `true` on success, `false` if the update fails. Idempotent — returns `true` immediately if the record is not deleted. Runs `before_restore`, the write, `restore!` on the `cascade:` dependents whose timestamp equals this record's, then `after_restore`, inside one transaction. |
