...
| `persist:` | `Boolean` or `Hash` | `false` | When truthy, a zone resolved from **`params[param]`** (an explicit user choice — never the header or the cookie itself) is written to the `cookie:` cookie as `{ value: zone.name, expires: 1.year.from_now }` — the `expires:` Duration is resolved to an absolute time at write time, because the cookie jar only coerces a Duration itself from Rails 5.2 on, and resolving it here keeps the written value independent of the jar. Pass a Hash to override/extend the cookie options (`expires:`, `same_site:`, `secure:`, `httponly:`, `domain:` …). Requires `cookie:`; `persist:` without it raises `ArgumentError` at declaration time. Cookie-varying responses must not be shared-cached: `Vary` advertises only `Time-Zone`, so a shared cache in front of a `cookie:`-enabled action could hand one visitor's zone to another — mark those responses private. |
| `response_header:` | `Boolean` or `String` | `false` | `true` emits the resolved zone's name in an `X-Time-Zone` response header; a String uses that header name instead. When the header source is enabled (`header: true`), `Time-Zone` is appended to the response's `Vary` header (case-insensitively, existing values kept) so caches key on the request header. Rails' own `Vary: Accept` is preserved: Rails only adds it while `Vary` is still blank, so this concern — which writes before the action — seeds it itself whenever Rails would have. A response already marked `Vary: *` is left untouched. |
...
- **The `cookie:` source now works on a real `ActionController::Base`.** `ActionController` declares `#cookies` **private**, so the original `respond_to?(:cookies)` guard was false there: the documented cookie source did nothing in any real Rails app, while the specs' public-`cookies` double made it look alive. Both cookie guards now ask `respond_to?(:cookies, true)`.
- **Cookie reading still requires the controller to have a cookie jar.** If the controller does not respond to `cookies` at all (e.g., an API-mode controller that does not include the cookie middleware), the cookie source is silently skipped. The same applies to `params` for the param source and `request` for the header source.
