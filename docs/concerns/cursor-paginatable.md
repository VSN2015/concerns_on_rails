...
- **No database columns required**, but ordering columns should be `NOT NULL`. A NULL in an ordering column fails loudly with a descriptive `ArgumentError` rather than silently dropping rows: on the page-boundary row; on the `limit+1` probe row just after it (the next page's strict `col > v` is never TRUE for a NULL); and on the last page of a cursor walk, when a NULL-ordered row the walk never returned sorts after the cursor's boundary (NULLs sort last ascending on PostgreSQL, last descending on SQLite/MySQL — exactly where the strict keyset comparison skips them). NULL rows that sort *before* every boundary were already returned and pass. The last-page check costs two small queries and runs only when an ordering column is nullable in the schema; NOT NULL columns pay nothing. Use NOT NULL columns or COALESCE in a view.
