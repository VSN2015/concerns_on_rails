...
- **No database columns required**, but ordering columns should be `NOT NULL`. The contract is exact: **if any row the relation would page over has a NULL in an ordering column, every page raises a descriptive `ArgumentError`** (`ordering column 'x' has NULL values in the paginated relation`) instead of silently dropping rows — the keyset predicate (`col > v`, `(col, id) > (v, x)`) is never TRUE for a NULL, so such rows would otherwise vanish, and which ones would depend on where the adapter sorts NULLs. Before each page, every ordering column the **schema** allows NULL in is probed with one `EXISTS (… WHERE table.col IS NULL)` over the relation's own conditions (ORDER/LIMIT/OFFSET/SELECT dropped, so DISTINCT and joined relations stay valid SQL) — one cheap, indexable query per nullable ordering column per page; NOT NULL columns (and the primary key) cost nothing. NULLs the relation's own `where` excludes don't count, so `scope.where.not(score: nil)` is the fix when NULL rows should simply be left out. Use NOT NULL columns or COALESCE in a view.
