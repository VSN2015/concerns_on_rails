...
Lookup is by **specificity** (no actions = catch-all): a rule that names the current action always beats a catch-all, whichever class declared either. Among the rules naming the action — and, if there are none, among the catch-alls — the most-derived declaring class wins, then declaration order within a class. So a parent's catch-all never shadows a subclass rule; a subclass catch-all for its new actions never takes over an action its parent named (nor drops that rule's `replay:`/`tolerance:`/`scheme:`); and a catch-all may be declared before specific rules in the same class — e.g. from a shared concern's `included` block. Invalid configuration raises `ArgumentError` at class-load time.

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `*actions` | `Symbol`s | none (catch-all) | Actions this rule covers. |
| `secret:` | `String`, callable, or `Array` | — (required) | Callables are resolved per request: a Proc/lambda is `instance_exec`'d on the controller (read `params` for multi-tenant secrets); any other callable (an object with `#call`, a `Method`) is passed the controller — or called bare when its `#call` takes no arguments; an Array means rotation — any match passes. Resolving **blank at request time raises `ArgumentError`**. |
...
| `replay:` | `true`, a store, or `false` | `nil` (off) | Replay protection. After the signature verifies, `SHA256(delivery identity)` is written to the store with `unless_exist:` and `replay_ttl:`; a second delivery with the same signature is rejected with 409 `webhook_replayed`. The identity is the signature header, except for `:stripe` where it is the signed `"#{t}.#{body}"` payload — the raw Stripe header is not canonical (the parser ignores unknown `v0=` keys and whitespace, so a captured header can be mutated into unlimited valid variants). `true` uses `ConcernsOnRails.config.cache_store` (raises with a setup hint when none is configured); any object with `#write(key, value, expires_in:, unless_exist:)` and `#read(key)` works — `#read` is **required**, because without it a falsy `#write` cannot be told apart from an unreachable store. `#delete(key)` is optional (it releases the claim whenever the delivery was not processed — the action 5xxed or raised, or a later `before_action` halted; without it, or if it raises — logged, never masking the original outcome — the 60-second claim expires on its own). `skip_after_action :commit_webhook_replay_claim` disables replay protection: the claim is never promoted, so every one is released. `false` is the same as `nil`, so `replay: Rails.env.production?` is safe. Forged traffic never consumes a slot; keys are scoped per controller action. |
...
- **Rule lookup is by specificity.** It used to be first-match over the inherited rules first, so a parent's catch-all shadowed every rule a subclass declared. A rule naming the action now always beats a catch-all (most-derived declaring class first, then declaration order), so subclass rules are no longer shadowed and inherited action-specific rules keep precedence over a subclass catch-all.
- **The replay claim is released whenever the delivery was not processed.** Rails skips `after_action` callbacks when a later `before_action` halts and when the action raises; the claim then lingered for 60 seconds and the provider's retry got a 409 for a delivery that never ran. A new private `around_action` (`guard_webhook_replay_claim`) releases it; a raising `#delete` is logged, never raised. `commit_webhook_replay_claim` is now private (it was public, and so a routable action method).
