...
| `candidates:` | Array / nil | `nil` | friendly_id slug candidates, tried in order until one is available: each entry is a Symbol/String (a method on the record), a Proc, or an Array of those (values joined with the sequence separator) — `[:title, %i[title city], %i[title city year]]`. Only when every candidate is taken does friendly_id fall back to the first candidate plus its uuid suffix. Regeneration is still driven by the primary `field` changing; a NULL slug backfills through the candidates. Must be a non-empty Array. Neither `field` nor a Symbol/String candidate may be an [Encryptable](encryptable.md) field — the slug would be its plaintext — which raises `ArgumentError` from either declaration order (a refused `sluggable_by` changes nothing); Encryptable also refuses such a save, covering an include of Sluggable without `sluggable_by` (the implicit `:name`). |
